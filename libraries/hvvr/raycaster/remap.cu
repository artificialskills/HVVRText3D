...
void save_image(std::string filename, int size, int hres, int vres, unsigned int* image)
{
	FILE *file;
	...
	int zeroDword = 0;
	fwrite(&zeroDword, 1, 4, file);
	int start = 54;
	fwrite(&start, 1, 4, file);
	int hdrsz = 40;
	fwrite(&hdrsz, 1, 4, file);
	fwrite(&hres, 1, 4, file);
	fwrite(&vres, 1, 4, file);
	short planes = 1;
	fwrite(&planes, 1, 2, file);
	short BPP = 32;
	fwrite(&BPP, 1, 2, file);
	fwrite(&zeroDword, 1, 4, file);
	fileSize -= 54;
...
void GPUCamera::remap() {
	...
	if (false)
	{
		uint32_t* h_imageData = new uint32_t[resultImage.width()* resultImage.height()];
		cudaMemcpy(h_imageData, d_imageData, resultImage.width()* resultImage.height() * sizeof(unsigned int), cudaMemcpyDeviceToHost);
		uint32_t* h_imageData_i = new uint32_t[resultImage.width()* resultImage.height()];
		for (int i = 0; i < resultImage.height(); i++)
		{
			for (int j = 0; j < resultImage.width(); j++)
			{
				h_imageData_i[i*resultImage.width() + j] = h_imageData[(resultImage.height() - i - 1)*resultImage.width() + j];
			}
		}

		char shit[8];
		itoa(fcount++, shit, 10);
		save_image("frame" + std::string(shit) + ".bmp", resultImage.width()* resultImage.height() * 4, resultImage.width(), resultImage.height(), h_imageData_i);
	}
}
